...
if (empty($id)) {
    setFlashMessage('ID kategori tidak valid', 'error');
...
try {
    ...
    if (!$kategori) {
        setFlashMessage('Kategori tidak ditemukan', 'error');
        redirect(SITE_URL . 'admin/kategori/index.php');
    }
    
    // Get jumlah buku dengan kategori ini
...
if (isset($_POST['update_kategori'])) {
    ...
    // Check if kategori name already exists (excluding current)
    if (empty($errors)) {
        try {
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE kategori SET nama_kategori = ?, deskripsi = ? 
                    WHERE id_kategori = ?";
            $stmt = $conn->prepare($sql);
            $result = $stmt->execute([$nama_kategori, $deskripsi, $id]);
            
            if ($result) {
                logActivity('UPDATE_KATEGORI', "Kategori diupdate: {$nama_kategori} (ID: {$id})", 'kategori', $id);
                setFlashMessage('Kategori berhasil diupdate!', 'success');
                redirect(SITE_URL . 'admin/kategori/index.php');
            } else {
                throw new Exception('Gagal mengupdate data kategori');
            }
        } catch (Exception $e) {
                ...
                <div class="card-body">
                    <form method="POST" id="kategoriForm" novalidate>
                        ...
                        <div class="d-flex justify-content-between pt-3">
                            ...
                            <button type="submit" name="update_kategori" class="btn btn-warning text-white btn-modern" id="submitBtn">
            ...
            <div class="modern-card">
                ...
                <div class="card-body">
                    <div class="text-center mb-4">
                        ...
                        <h5><?= htmlspecialchars($kategori['nama_kategori']) ?></h5>
                    ...
                    <?php if ($jumlah_buku > 0): ?>
                        ...
                        <div class="alert alert-info small mb-0">
                            ...
                            Perubahan nama kategori akan mempengaruhi <?= $jumlah_buku ?> buku.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="modern-card">
                ...
                <div class="card-body">
                    <table class="table table-sm table-borderless">
                        <tr>
                            <td class="fw-semibold" style="width: 40%">ID Kategori:</td>
                            <td class="text-end"><code><?= $id ?></code></td>
                        </tr>
                        <tr>
                            <td class="fw-semibold">Deskripsi:</td>
                            <td class="text-end"><?= $kategori['deskripsi'] ? htmlspecialchars($kategori['deskripsi']) : '<em class="text-muted">Belum ada deskripsi</em>' ?></td>
                        </tr>
                        <tr>
                            <td class="fw-semibold">Jumlah Buku:</td>
                            <td class="text-end">
                                <span class="badge bg-<?= $jumlah_buku > 0 ? 'primary' : 'secondary' ?>">
                                    <?= $jumlah_buku ?> buku
                                </span>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="text-end"><?= formatTanggal($kategori['created_at']) ?></td>
                        </tr>
                    </table>
                </div>
            </div>
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <?php if ($total_stok == 0): ?>
                         ...
                         <?php else: ?>
                            ...
                            <small class="text-muted text-center">Kategori memiliki <?= $jumlah_buku ?> buku</small>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
