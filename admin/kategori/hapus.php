...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM buku_kategori WHERE id_kategori = ?");
        $stmt->execute([$id]);
        $book_count = $stmt->fetchColumn();
        
        if ($book_count > 0) {
            throw new Exception("Tidak dapat menghapus kategori yang memiliki {$book_count} buku!");
        }
        ...
        // Log activity
        logActivity('HAPUS_KATEGORI', "Kategori dihapus: {$kategori['nama_kategori']} (ID: {$id})", 'kategori', $id);
        
        redirect('index.php', "Kategori '{$kategori['nama_kategori']}' berhasil dihapus!", 'success');
...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    redirect('index.php', 'ID kategori tidak valid!', 'error');
}
...
try {
    ...
    if (!$kategori) {
        redirect('index.php', 'Kategori tidak ditemukan!', 'error');
    }
    
    // Check if kategori has books
    $stmt = $conn->prepare("SELECT COUNT(*) as jumlah_buku FROM buku_kategori WHERE id_kategori = ?");
    $stmt->execute([$id]);
    ...
    <div class="delete-container">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="index.php">Kelola Kategori</a></li>
            ...
            <div class="error-card">
                ...
                <div class="detail-card-body">
                    <div class="error-alert">
                        <strong><i class="fas fa-exclamation-circle me-2"></i>Kategori ini memiliki <?= $book_info['jumlah_buku'] ?> buku!</strong>
                        <br>
                        Kategori tidak dapat dihapus sampai semua buku dipindahkan ke kategori lain.
                    </div>
                    ...
                    <div class="alert alert-warning">
                        ...
                        <ul class="mt-2 mb-0">
                            <li>Pindahkan semua buku ke kategori lain terlebih dahulu</li>
                            <li>Atau hapus kategori dari semua buku yang menggunakannya</li>
                        </ul>
                    </div>
        ...
        <?php else: ?>
            ...
            <div class="detail-card">
                <div class="detail-card-header">
                    <h5><i class="fas fa-tag me-2"></i>Detail Kategori yang Akan Dihapus</h5>
                </div>
                <div class="detail-card-body">
                    ...
                    <div class="avatar-section">
                        ...
                        <div class="avatar-info">
                            <h4><?= htmlspecialchars($kategori['nama_kategori']) ?></h4>
                            <span class="role-badge-delete">KATEGORI</span>
                    ...
                    <table class="info-table">
                        ...
                        <tr>
                            ...
                            <td><span class="info-value"><?= htmlspecialchars($kategori['nama_kategori']) ?></span></td>
                        </tr>
                        <tr>
                            <th><i class="fas fa-align-left me-2"></i>Deskripsi</th>
                            <td><?= $kategori['deskripsi'] ? htmlspecialchars($kategori['deskripsi']) : '<em class="text-muted">-</em>' ?></td>
                        </tr>
                        <tr>
                            <th><i class="fas fa-calendar me-2"></i>Ditambahkan</th>
                            <td><?= formatTanggal($kategori['created_at']) ?></td>
                        </tr>
                    </table>
                    ...
                    <div style="background: #d1ecf1; border-left: 4px solid #17a2b8; padding: 1rem 1.5rem; border-radius: 10px; margin-top: 1.5rem; margin-bottom: 1.5rem;">
                        ...
                        <strong style="color: #0c5460;">Status Kategori:</strong>
                        <p style="color: #0c5460; margin: 0.5rem 0 0 0; margin-bottom: 0;">
                            Kategori ini <strong>tidak memiliki buku</strong> dan dapat dihapus dengan aman.
                        </p>
                    </div>
                    ...
                    <form method="POST" id="deleteForm">
                        ...
                        <div class="d-flex gap-3 justify-content-between">
                            ...
                            <button type="submit" name="confirm_delete" class="btn-delete" id="deleteButton" disabled>
                                <i class="fas fa-trash me-2"></i>Hapus Kategori Sekarang
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <div class="final-warning">
                ...
                <p class="mb-2">Dengan menghapus kategori ini, Anda akan kehilangan:</p>
                <ul>
                    <li>Data kategori secara permanen</li>
                    <li>Informasi deskripsi kategori</li>
                    <li>Akses untuk memilih kategori ini di buku baru</li>
                </ul>
            </div>
        <?php endif; ?>
    </div>
...
document.getElementById('deleteForm')?.addEventListener('submit', function(e) {
    const namaKategori = '<?= htmlspecialchars($kategori['nama_kategori'], ENT_QUOTES) ?>';
    
    if (!confirm(`Apakah Anda BENAR-BENAR YAKIN ingin menghapus kategori "${namaKategori}"?\n\nTindakan ini TIDAK DAPAT DIBATALKAN!`)) {
        e.preventDefault();
        return false;
    }
});
